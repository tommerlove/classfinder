<?php
    ...
    $sql = "SELECT * FROM teacher WHERE id = '$session_id' ";
    ...
    $sql2 = "SELECT * FROM group WHERE org = '$sec' ";
    $que2 = mysqli_query($conn,$sql2);
    $res2 = mysqli_fetch_array($que2);
    $department = $res2['group_name'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    Swal.fire({
      ...
      if (result.value) {
        Swal.fire({
          ...
          html: 'ระบบจะพาคุณไปยังหน้าเว็บไซต์<br>หวังเป็นอย่างยิ่งว่าระบบที่พัฒนาขึ้นนี้จะเป็นตัวกลางในการติดต่อระหว่าง ครู ผู้ปกครอง และนักเรียนได้เป็นอย่างดีครับ',
          icon: 'success'
        }).then(function() {
